...
Supports OpenAI, Gemini, and DeepSeek with automatic fallback.
"""
...
class LLMProvider(ABC):
    ...
    def evaluate_deal(self, title: str, price: float) -> str:
        """Evaluate a deal and return a brief assessment."""
        pass
...
class OpenAIProvider(LLMProvider):
    ...
    def evaluate_deal(self, title: str, price: float) -> str:
        """Evaluate deal using GPT-3.5-turbo."""
    ...
    def answer_question(self, question: str, context: str) -> str:
        """Answer question using GPT-3.5-turbo."""
        if not self.is_available():
            return "OpenAI error: OPENAI_API_KEY not configured."
...
def _get_available_provider(preferred: str = None) -> LLMProvider:
    """
    Get an available provider, trying preferred first, then fallback.
    
    Args:
        preferred: Preferred provider name ("openai", "gemini", or "deepseek")
    
    Returns:
        LLMProvider: An available provider or OpenAI as last resort
    """
...
def gpt_evaluate_deal(title: str, price: float, provider: str = None) -> str:
    """
    Evaluate a deal using the selected LLM provider.
    ...
    Returns:
        str: Evaluation result or error message
    """
...
def gpt_answer(question: str, context: str, provider: str = None) -> str:
    ...
    Args:
        ...
        context: Context information (deals)
        provider: Provider name override (optional)
